...
export const archiveService = {
  ...
  getDocuments: async (filters, page = 1, pageSize = 100) => {
    const { data, error } = await supabase.rpc("get_archived_documents_fs", {
      ...
      p_semester:
        filters.semester === "All Semesters" ? "ALL" : filters.semester,
      p_academic_year:
        filters.academic_year === "All Years" ? "ALL" : filters.academic_year,
      p_department: "ALL", // Hardcoded since system is single-department
      p_doc_type:
        filters.doc_type === "All Document Types" ? "ALL" : filters.doc_type,
   ...
   */
  getOptions: async () => {
    const [types, courses, faculty, semesterPeriods, systemSettings] =
      await Promise.all([
        supabase.from("documenttypes_fs").select("type_name"),
        supabase
          .from("courses_fs")
          .select("semester, academic_year, course_code, section, faculty_id"),
        supabase
          .from("faculty_fs")
    ...
    const uniqueCourses = [
      ...new Set(rawCourses.map((c) => c.course_code)),
    ].filter(Boolean);
    const uniqueSections = [
      ...new Set(rawCourses.map((c) => c.section)),
    ].filter(Boolean);

    // Sort faculties by last name
    const facultiesList = (faculty.data || [])
      .map((f) => ({
        id: f.faculty_id,
  ...
  downloadArchiveZip: async (config, onProgress, abortSignal) => {
    try {
      ...
      const { data: files, error } = await supabase.rpc(
        ...
        {
          p_semester:
            config.semester === "All Semesters" ? "ALL" : config.semester,
          p_academic_year:
            config.academic_year === "All Years" ? "ALL" : config.academic_year,
          p_faculty_id:
            config.faculty === "All Faculty" ? "ALL" : config.faculty,
          p_course_code:
            config.course === "All Courses" ? "ALL" : config.course,
          p_section: config.section === "All Sections" ? "ALL" : config.section,
          p_doc_type:
            config.doc_type === "All Document Types" ? "ALL" : config.doc_type,
        },
      );
      ...
      const payloadFiles = files.map((file) => {
        ...
        const facultyFolder = file.faculty_name
          ? file.faculty_name.replace(/[\/\\:*?"<>|]/g, "").trim()
          : "Unknown_Faculty";
        const courseFolder =
          `${file.course_code || "Course"} - ${file.section || "Sec"}`
            .replace(/[\/\\:*?"<>|]/g, "")
            .trim();
        ...
        const fileIdMatch =
          ...
          file.gdrive_download_link?.match(/\/d\/([a-zA-Z0-9-_]+)/);
      ...
      const precalculatedTotalBytes = files.reduce(
      ...
      return {
        ...
        message:
          err.message ||
          "Export failed. Please ensure the backend server is running.",
      };
    }
  },
