...
export const validationService = {
  ...
  getQueue: async (filters) => {
    ...
    const { data, error } = await supabase.rpc('get_validation_queue_fn', {
   ...
   */
  getStats: async () => {
    const { data, error } = await supabase.rpc('get_validation_stats_fn');
    if (error) throw error;
    return data;
  },
  ...
  processAction: async (submissionId, action, remarks) => {
    const { data, error } = await supabase.rpc('process_validation_action_fn', {
