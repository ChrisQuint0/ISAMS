...
export const FacultyResourceService = {
  ...
  async getTemplates() {
    try {
      ...
      const { data, error } = await supabase.rpc("get_available_templates_fs", {
  ...
  async getArchivedSubmissions(semester, academicYear) {
    try {
      ...
      if (academicYear)
        query = query.eq("courses_fs.academic_year", academicYear);
  ...
  async cloneDocument(
    ...
    try {
      ...
      const { data: oldSubmission, error: fetchErr } = await supabase
        .from("submissions_fs")
        .select(
          "doc_type_id, original_filename, standardized_filename, file_size_bytes, mime_type, gdrive_download_link, gdrive_web_view_link",
        )
        .eq("submission_id", oldSubmissionId)
        .single();

      if (fetchErr || !oldSubmission)
        throw new Error("Failed to fetch original document metadata");

      // Extract the original fileId from the link
      const fileIdMatch =
        oldSubmission.gdrive_download_link?.match(/id=([^&]+)/);
      if (!fileIdMatch)
        throw new Error("Original document missing Google Drive link");
      ...
      if (!rootFolderId)
        throw new Error(
          "Google Drive folder not configured. Please set it in Admin Settings.",
        );
      ...
      const clonedDriveFile = await cloneGDriveFile(
        oldFileId,
        targetFolderId,
        newFileName,
      );
      ...
      const { data: insertData, error: insertError } = await supabase.rpc(
        "upsert_submission_with_versioning_fs",
        {
          ...
          p_gdrive_download_link:
            clonedDriveFile.webContentLink || clonedDriveFile.webViewLink,
        },
      );
