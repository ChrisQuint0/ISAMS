...
export const facultyMonitorService = {
  ...
  getMonitoringData: async (filters) => {
    const { data, error } = await supabase.rpc("get_faculty_monitoring_fs", {
      p_semester:
        filters.semester === "All Semesters" ? null : filters.semester,
      p_academic_year:
        filters.academic_year === "All Years" ? null : filters.academic_year,
  ...
  getOptions: async () => {
    ...
    const currentSem = settings?.find(
      (s) => s.setting_key === "current_semester",
    )?.setting_value;
    const currentYear = settings?.find(
      (s) => s.setting_key === "current_academic_year",
    )?.setting_value;
  ...
  sendBulkReminders: async (
    ...
    message = "Please check your pending submissions.",
  ) => {
    ...
    const results = await facultyMonitorService.sendBulkEmails(
      filteredFacultyList,
      {
        subject,
        message,
  ...
  exportToCSV: (data, filename = "faculty_monitoring_report.csv") => {
    ...
    const rows = data.map((f) => [
      ...
      f.late_submissions,
      escapeCsv(
        f.courses ? f.courses.map((c) => c.course_code).join("; ") : "N/A",
      ),
    ]);
  ...
  adminUploadSubmission: async ({ file, facultyId, courseId, docTypeId }) => {
    try {
      ...
      const technicalFolder =
        docType?.gdrive_folder_name || docType?.type_name || "Other";
      ...
      if (!rootFolderId)
        throw new Error("GDrive root folder not configured in Admin Settings.");
      ...
      const renamedFile = new File([file], standardizedName, {
        type: file.type,
      });
      ...
      const { data, error: rpcError } = await supabase.rpc(
        ...
        {
          p_faculty_id: String(facultyId),           // faculty_id column is text
          ...
          p_gdrive_download_link:
            gdriveFile.webContentLink || gdriveFile.webViewLink,
          p_is_staged: false,
        },
      );
  ...
  requestRevision: async ({
    ...
    const ids = (submissionIds || (submissionId ? [submissionId] : [])).filter(
      (id) => id && /^\d+$/.test(String(id)),
    );
    ...
    if (manualUploads.length > 0 && courseId && docTypeId) {
      ...
      const existingGdriveIds = new Set(
        existingRecords?.map((r) => r.gdrive_file_id) || [],
      );
      ...
      for (const m of manualUploads) {
        if (existingGdriveIds.has(m.gdrive_file_id)) {
          const matchedSub = existingRecords.find(
            (r) => r.gdrive_file_id === m.gdrive_file_id,
          );
          ...
          if (!targetDocTypeId) {
            console.error(
              "[Service] Skipping manual upload insertion: Missing docTypeId",
              m,
            );
            continue;
          }
          ...
          toInsert.push({
            ...
            standardized_filename:
              m.standardized_filename || m.original_filename,
      ...
      if (toInsert.length > 0) {
        ...
        if (created)
          created.forEach((c) => finalIdsToUpdate.push(c.submission_id));
      }
    }
    ...
    const { error: notifError } = await supabase
      ...
      .insert({
        ...
        message:
          reason ||
          "Please review and resubmit your document — the admin has requested a revision.",
      });
  ...
  sendBulkEmails: async (
    facultyList,
    { subject, message, template = "deadline_reminder" } = {},
  ) => {
    ...
    const succeeded = results.filter(
      (r) => r.status === "fulfilled" && !r.value.ignored,
    ).length;
  ...
  syncSubmissionsWithGDrive: async (submissionIds) => {
    ...
    try {
      ...
      const { data: dbRecords, error: dbError } = await supabase
        .from("submissions_fs")
        .select(
          "submission_id, gdrive_file_id, submitted_at, submission_status",
        )
      ...
      for (const record of dbRecords) {
        ...
        try {
          ...
          console.warn(
            `[Sync] Failed to fetch metadata for ${record.gdrive_file_id}:`,
            err.message,
          );
        }
      }
          ...
          updates.map((u) =>
            supabase
              ...
              .update({
                ...
                approval_remarks:
                  "Automatically detected resubmission via GDrive file update.",
              })
