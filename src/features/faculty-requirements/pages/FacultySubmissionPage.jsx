...
export default function FacultySubmissionPage() {
  ...
  // Current "Today" for the app — mocked for testing as per user's requests in other files
  const today = new Date();
  today.setHours(0, 0, 0, 0);

  const checkIsBlocked = (doc) => {
    if (!doc) return false;
    if (doc.is_blocked) return true; // Server already confirmed blocked

    if (doc.deadline_date) {
      const grace = doc.grace_period_days || 0;
      const cutoffDate = new Date(doc.deadline_date);
      cutoffDate.setDate(cutoffDate.getDate() + grace);
  ...
  const checkIsLate = (doc) => {
    if (!doc || !doc.deadline_date) return false;
    const deadlineDate = new Date(doc.deadline_date);
    deadlineDate.setHours(23, 59, 59, 999);
    return today > deadlineDate;
  };

  const selectedDocInfo = requiredDocs.find(d => String(d.doc_type_id) === String(formData.documentType));
  const isBlocked = checkIsBlocked(selectedDocInfo);
  const isLate = !isBlocked && checkIsLate(selectedDocInfo);
      ...
      <div className="space-y-6 flex flex-col h-full bg-neutral-50/30">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    >
                      ...
                      <SelectContent className="bg-white border-neutral-200 text-neutral-900">
                        {requiredDocs.map(doc => {
                          const docBlocked = checkIsBlocked(doc);
                          const docLate = !docBlocked && checkIsLate(doc);
                          return (
                            <SelectItem
                              key={doc.doc_type_id}
                              value={doc.doc_type_id.toString()}
                              className={`font-medium text-sm ${doc.is_submitted
                                ? "text-success focus:text-success focus:bg-success/10"
                                : docBlocked
                                  ? "text-neutral-400 focus:text-neutral-400"
                                  : docLate
                                    ? "text-warning focus:text-warning focus:bg-warning/10"
                                    : "text-neutral-900 focus:text-neutral-900 focus:bg-neutral-100"
                                }`}
                            >
                              {doc.type_name} {doc.is_required === false ? "(Optional)" : ""} {doc.is_submitted ? "(Submitted)" : ""} {docBlocked ? "(Closed)" : docLate ? "(Late)" : ""}
                            </SelectItem>
                          );
                        })}
                      </SelectContent>
                              ...
                              <div className="flex items-center gap-2 mt-1 flex-wrap">
                                ...
                                {stagedFile.validating ? (
                                  <span className="inline-flex items-center text-xs font-bold text-info ml-2">
                                    <Loader2 className="h-3 w-3 mr-1 animate-spin" /> Analyzing OCR...
                                  </span>
                                ) : stagedFile.error ? (
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-2 border-t border-neutral-100">
                  ...
                  >
                    {isSubmitting ? (
                      ...
                      <>
                        <Loader2 className="h-4 w-4 mr-2 animate-spin" /> Validating Document(s)...
                      </>
                    ) : (
                      <>
          ...
          <Dialog open={showPreview} onOpenChange={setShowPreview}>
            <DialogContent className="max-w-4xl h-[85vh] p-0 overflow-hidden flex flex-col bg-neutral-100">
              ...
              <div className="flex-1 overflow-auto p-4 flex items-center justify-center">
                {!previewUrl ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-neutral-500 text-center max-w-sm mx-auto p-8 bg-white rounded-xl shadow-sm border border-neutral-200">
                    <FileText className="h-16 w-16 text-neutral-300 mb-4" />
                    <p className="font-bold text-neutral-900 text-lg mb-2">No Preview Available</p>
                    <p className="text-sm font-medium">
                      This file type ({previewFile?.type}) cannot be previewed directly in the browser. You can still submit it.
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="flex flex-col gap-6 h-full">
            <Card className="bg-white border-neutral-200 shadow-sm overflow-hidden flex flex-col flex-1">
              ...
              <CardContent className="pt-4 px-4 space-y-3 bg-white max-h-[500px] overflow-y-auto custom-scrollbar flex-1">
                {loadingHistory ? (
