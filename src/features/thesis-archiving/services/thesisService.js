...
export const thesisService = {
  ...
  /**
   * Add a new thesis adviser
   */
  async addAdviser(adviser) {
    const { data, error } = await supabase
  ...
  async getThesisEntries(filters = {}) {
    ...
    if (filters.search) {
      ...
      query = query.or(
        `title.ilike.${searchTerm},description.ilike.${searchTerm},abstract.ilike.${searchTerm}`,
      );
    }
  ...
  async deleteSection(id) {
    const { error } = await supabase.from("hte_sections").delete().eq("id", id);

    if (error) throw error;
  },
  async createHTEStudent({
    studentData,
    password,
    academicYear,
    semester,
  ...
  async batchCreateHTEStudents({
    ...
    const data = await response.json();
    if (!response.ok)
      throw new Error(data.error || "Failed to process batch creation");
    return data.results;
  },
    ...
    try {
      ...
      if (!error && data) {
        ...
        return {
          ...data,
          archived_this_year:
            data.archived_this_year || data.archived_this_term || 0,
      ...
      const [total, thisYear] = await Promise.all([
        ...
        supabase
          ...
          .select("id", { count: "exact", head: true })
          .eq("is_deleted", false)
          .eq("publication_year", currentYear),
      ]);
   ...
   */
  async getSettings() {
    const { data, error } = await supabase.from("thesis_settings").select("*");
  ...
  async updateSetting(key, value, userId) {
    ...
    if (existing) {
      ...
      const { error } = await supabase.from("thesis_settings").insert([
        {
          key,
          value: String(value),
          updated_by: userId,
  ...
  async sendBatchNotifications(batchData, actorInfo = {}) {
    ...
    const data = await response.json();
    if (!response.ok)
      throw new Error(data.error || "Failed to send batch notifications");
    return data;
  },
