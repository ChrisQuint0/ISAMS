...
export const similarityService = {
  ...
  async extractText(file) {
    ...
    const res = await fetch(getApiUrl("/api/similarity/extract"), {
      method: "POST",
      body: formData,
    });
  ...
  async createScanJob({
    ...
    const res = await fetch(getApiUrl("/api/similarity/job"), {
      method: "POST",
  ...
  async runAnalysis({
    ...
    const res = await fetch(getApiUrl("/api/similarity/analyze"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        scanId,
  ...
  async fetchScanResult(scanId) {
    const res = await fetch(getApiUrl(`/api/similarity/result/${scanId}`));
    const data = await res.json();
  ...
  async fetchRecentScans() {
    ...
    const res = await fetch(getApiUrl(`/api/similarity/recent/${user.id}`));
    const data = await res.json();
  ...
  async getThreshold() {
    try {
      const res = await fetch(getApiUrl("/api/similarity/threshold"));
      const data = await res.json();
  ...
  async markAsReviewed(
    scanResultId,
    { reviewStatus, actionTaken, notes, actorInfo = {} },
  ) {
    const res = await fetch(getApiUrl("/api/similarity/review"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        scanResultId,
  ...
  exportPDF({ queueRow, result, fieldScores, topMatches }) {
    ...
    doc.text(`Generated: ${new Date().toLocaleString()}`, pageW - 15, 12, {
      align: "right",
    });
    y = 28;
    ...
    addLine(
      queueRow?.proposed_title || queueRow?.original_filename || "Untitled",
      12,
      true,
    );
    addLine(
      `File: ${queueRow?.original_filename || "N/A"}  |  Scanned: ${new Date(queueRow?.submitted_at).toLocaleDateString()}`,
      9,
      false,
      [100, 100, 100],
    );
    ...
    if (topMatches?.length > 0) {
      ...
      topMatches.slice(0, 5).forEach((m, i) => {
        ...
        const authorsStr = Array.isArray(m.matched_authors)
          ? m.matched_authors.join(", ")
          : m.matched_authors || "";
        doc.text(
          `${authorsStr}${m.matched_year ? ` • ${m.matched_year}` : ""}  |  Matched in: ${m.match_type || "Content"}`,
          18,
          y,
        );
        y += 6;
    ...
    doc.setFont("helvetica", "normal");
    doc.text(
      `Engine: ${result?.analysis_method || "ISAMS-NLP v1.0"}  |  Analysis Time: ${result?.analysis_duration_ms ? (result.analysis_duration_ms / 1000).toFixed(2) + "s" : "N/A"}  |  Repository: ${queueRow?.repository_size_at_scan ?? "N/A"} records`,
      15,
      y,
    );
