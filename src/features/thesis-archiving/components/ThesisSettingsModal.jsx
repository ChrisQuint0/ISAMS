...
const ActionCellRenderer = (params) => {
  ...
  useEffect(() => {
    ...
    const isRowEditing = editingCells.some(
  ...
  return (
    <div className="flex items-center justify-center w-full h-full gap-2">
      {params.context && params.context.view === "years" && (
        ...
        >
          <Check
            className={`h-4 w-4 ${params.data.is_active ? "stroke-[3px]" : ""}`}
          />
        </button>
      )}
      <button
        ...
        title={
          isEditing ? "Save changes" : isRecentSave ? "Saved" : "Delete item"
        }
      >
        {isEditing ? (
          ...
          <Trash2
            className={`h-4 w-4 ${isRecentSave ? "text-neutral-400" : "text-red-500"}`}
          />
        )}
      </button>
    </div>
...
export function ThesisSettingsModal({ variant = "dark" }) {
  ...
  // Form states
  const [newAdviserFirstName, setNewAdviserFirstName] = useState("");
  const [newAdviserLastName, setNewAdviserLastName] = useState("");
  const [newAdviserTitle, setNewAdviserTitle] = useState("");
  const [newAdviserCredentials, setNewAdviserCredentials] = useState("");
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (settingsData.thesis_root_folder_id) {
        setThesisFolderLink(
          `https://drive.google.com/drive/folders/${settingsData.thesis_root_folder_id}`,
        );
      }
      if (settingsData.hte_parent_folder_id) {
        setHteFolderLink(
          `https://drive.google.com/drive/folders/${settingsData.hte_parent_folder_id}`,
        );
      }
    } catch (error) {
  ...
  const handleAddAdviser = async () => {
    const firstName = newAdviserFirstName.trim();
    const lastName = newAdviserLastName.trim();
    const title = newAdviserTitle.trim();
    const credentials = newAdviserCredentials.trim();

    if (!firstName || !lastName) return;

    try {
      await thesisService.addAdviser({
      ...
      setNewAdviserLastName("");
      setNewAdviserTitle("");
      setNewAdviserCredentials("");
    } catch (error) {
      console.error("Failed to add adviser:", error);
    }
  };
    ...
    if (itemToDelete) {
      try {
        if (view === "advisers") {
          ...
          setAcademicYears(
            academicYears.filter((y) => y.id !== itemToDelete.id),
          );
        }
      } catch (error) {
  ...
  const handleCellValueChanged = async (event) => {
    if (view === "advisers") {
      ...
      try {
        ...
        setAcademicYears((prev) =>
          prev.map((y) => (y.id === event.data.id ? event.data : y)),
        );
      } catch (error) {
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      await Promise.all([
        thesisService.updateSetting(
          "thesis_root_folder_id",
          thesisFolderId,
          user?.id,
        ),
        thesisService.updateSetting(
          "hte_parent_folder_id",
          hteFolderId,
          user?.id,
        ),
      ]);
        ...
        {view === "settings" ? (
          <>
            ...
            <div className="grid gap-6 flex-1">
              <div className="flex items-center gap-4">
                <Label
                  htmlFor="thesis-link"
                  className="text-base w-48 text-neutral-900"
                >
                  Thesis Folder Link
                </Label>
                <Input
            ...
            <div className="flex justify-end pt-2 border-t border-neutral-200 mt-auto items-center gap-3">
              {isSaved && (
                <span className="text-emerald-600 text-sm font-medium animate-in fade-in slide-in-from-right-2">
                  Saved successfully!
                </span>
              )}
              <Button
          ...
          <div className="flex flex-col h-full gap-4">
            <DialogHeader>
              ...
              <DialogDescription className="text-neutral-600 text-sm">
                These advisers would appear as an option in the adviser dropdown
                when adding a new research entry.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <Input
                ...
                value={newAdviserFirstName}
                onChange={(e) => setNewAdviserFirstName(e.target.value)}
                className="bg-white border-neutral-200 text-neutral-900 focus-visible:ring-neutral-300"
              />
              <Input
                placeholder="Last Name"
                value={newAdviserLastName}
                onChange={(e) => setNewAdviserLastName(e.target.value)}
                className="bg-white border-neutral-200 text-neutral-900 focus-visible:ring-neutral-300"
              />
              <Input
                placeholder="Title (e.g., Prof., Dr., Asst. Prof.)"
                value={newAdviserTitle}
                onChange={(e) => setNewAdviserTitle(e.target.value)}
                className="bg-white border-neutral-200 text-neutral-900 focus-visible:ring-neutral-300"
              />
              <Input
                placeholder="Credentials (e.g., Ph.D., MIT)"
                value={newAdviserCredentials}
                onChange={(e) => setNewAdviserCredentials(e.target.value)}
                className="bg-white border-neutral-200 text-neutral-900 focus-visible:ring-neutral-300"
                onKeyDown={(e) => {
          ...
          <div className="flex flex-col h-full gap-4">
            <DialogHeader>
              ...
              <DialogDescription className="text-neutral-600 text-sm">
                These categories would appear as an option in the category
                dropdown when adding a new research entry.
              </DialogDescription>
            </DialogHeader>
          ...
          <div className="flex flex-col h-full gap-4">
            <DialogHeader>
              ...
              <DialogDescription className="text-neutral-600 text-sm">
                Manage class sections for Computer Science and Information
                Technology.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="flex gap-2 items-center">
              ...
              />
              <Select
                value={newSectionProgram}
                onValueChange={setNewSectionProgram}
              >
