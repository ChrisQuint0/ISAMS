...
// ─────────────────────────────────────────────────────────────
const ACCENT = {
  thesis:     { gradFrom: "from-cyan-400",    gradTo: "to-blue-500",    text: "text-cyan-400",    border: "border-cyan-500/25",    activeBg: "bg-cyan-500/6"    },
  similarity: { gradFrom: "from-violet-400",  gradTo: "to-indigo-500",  text: "text-violet-400",  border: "border-violet-500/25",  activeBg: "bg-violet-500/6"  },
  ojt:        { gradFrom: "from-emerald-400", gradTo: "to-teal-500",    text: "text-emerald-400", border: "border-emerald-500/25", activeBg: "bg-emerald-500/6" },
};
...
function Panel({ children, className = "" }) {
  return (
    <div className={`rounded-2xl border border-slate-800/60 bg-slate-900/35 p-7 ${className}`}>
      {children}
    </div>
  );
}
...
function SectionTitle({ children, count, reportType = "thesis" }) {
  ...
  return (
    <div className="flex items-center justify-between mb-7">
      ...
      {count !== undefined && (
        <span className="font-mono text-xs text-slate-600 bg-slate-950/70 border border-slate-800 px-3 py-1.5 rounded-full">
          {count}
        </span>
      )}
    </div>
...
function ProgressRow({ label, sub, pct, displayValue, colorClass, valueClass }) {
  return (
    <div className="group flex items-center gap-5 py-3 px-3 rounded-xl hover:bg-slate-800/25 transition-colors cursor-default">
      <div className="w-36 shrink-0">
        <p className="text-sm font-semibold text-slate-300 group-hover:text-white transition-colors leading-tight">{label}</p>
        {sub && <p className="text-xs text-slate-600 mt-0.5 font-medium">{sub}</p>}
      </div>
...
function Pagination({ page, totalPages, onPage }) {
  ...
  return (
    <div className="flex items-center justify-between pt-5 mt-5 border-t border-slate-800/50">
      <p className="text-xs text-slate-600 font-mono">Page {page} of {totalPages}</p>
      <div className="flex items-center gap-1.5">
        <Button variant="outline" size="sm" disabled={page === 1} onClick={() => onPage(page - 1)}
          className="h-8 w-8 p-0 text-base border-slate-800 text-slate-500 bg-transparent hover:text-white hover:bg-slate-800 hover:border-slate-700 disabled:opacity-20 transition-all rounded-lg"
        >‹</Button>
        {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
          <Button key={p} size="sm" onClick={() => onPage(p)}
            className={`h-8 w-8 p-0 text-xs font-bold transition-all rounded-lg ${
              p === page
                ? "bg-gradient-to-br from-cyan-500 to-blue-600 text-white border-0 shadow-[0_2px_14px_rgba(34,211,238,0.35)]"
                : "bg-transparent border border-slate-800 text-slate-600 hover:text-white hover:bg-slate-800 hover:border-slate-700"
            }`}
          >{p}</Button>
        ))}
...
function ExportToolbar({ onExcel, onCSV, onPDF, exporting }) {
  ...
  const btns = [
    { key:"excel", label:"XLSX", fn:onExcel, cls:"from-emerald-700 to-emerald-500 hover:from-emerald-600 hover:to-emerald-400 shadow-[0_2px_8px_rgba(16,185,129,0.2)] hover:shadow-[0_2px_18px_rgba(16,185,129,0.35)]" },
    { key:"csv",   label:"CSV",  fn:onCSV,   cls:"from-blue-700 to-blue-500 hover:from-blue-600 hover:to-blue-400 shadow-[0_2px_8px_rgba(59,130,246,0.2)] hover:shadow-[0_2px_18px_rgba(59,130,246,0.35)]" },
    { key:"pdf",   label:"PDF",  fn:onPDF,   cls:"from-rose-700 to-rose-500 hover:from-rose-600 hover:to-rose-400 shadow-[0_2px_8px_rgba(244,63,94,0.2)] hover:shadow-[0_2px_18px_rgba(244,63,94,0.35)]" },
  ];
  return (
    <div className="flex items-center gap-2">
      <span className="text-[10px] font-bold uppercase tracking-widest text-slate-700 mr-1">Export as</span>
      {btns.map(({ key, label, fn, cls }) => (
        <Button key={key} size="sm" disabled={!!exporting} onClick={fn}
          className={`h-8 px-4 text-xs font-bold bg-gradient-to-r ${cls} text-white border-0 transition-all disabled:opacity-40`}
        >
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-slate-950/60">
            {columns.map((col, i) => (
              <th key={i} className="px-5 py-3.5 text-left text-[10px] font-bold uppercase tracking-widest text-slate-600 whitespace-nowrap border-b border-slate-800/50">
                {col}
              </th>
            ))}
          </tr>
...
function CategoryBadge({ category }) {
  const map = {
    ...
    "Capstone Project": "bg-blue-500/10 text-blue-400 border-blue-500/20",
  };
  return (
    <Badge className={`border text-xs font-semibold px-2.5 py-0.5 rounded-lg ${map[category] ?? "border-slate-700 text-slate-500"}`}>
      {category}
    </Badge>
  );
}
...
function ReportSkeleton() {
  return (
    <div className="space-y-5">
      {[5, 8].map((rows, ci) => (
        <div key={ci} className="rounded-2xl border border-slate-800/50 bg-slate-900/30 p-7 space-y-5">
          <Skeleton className="h-6 w-52 bg-slate-800/80 rounded-lg" />
          {Array.from({ length: rows }, (_, i) => (
            <div key={i} className="flex items-center gap-5">
              <Skeleton className="h-4 w-32 bg-slate-800/80 rounded" />
              <Skeleton className="flex-1 h-2 bg-slate-800/80 rounded-full" />
              <Skeleton className="h-4 w-12 bg-slate-800/80 rounded" />
            </div>
          ))}
        </div>
      ))}
...
// ─────────────────────────────────────────────────────────────
const inputCls =
  "h-10 text-sm bg-slate-950/70 border-slate-800 text-slate-100 placeholder:text-slate-700 " +
  "focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20 hover:border-slate-700 transition-colors rounded-xl";

const triggerCls =
  "h-10 text-sm bg-slate-950/70 border-slate-800 text-slate-100 " +
  "focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20 hover:border-slate-700 transition-colors rounded-xl";

const dropdownCls = "bg-slate-950 border-slate-800 text-slate-100 rounded-xl";
const labelCls    = "text-[10px] font-bold uppercase tracking-widest text-slate-600 mb-2 block";
...
function ReportsFilters({ onFilterChange, showOJTFilters = false, reportType = "thesis" }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-800/60 bg-slate-900/30">
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800/50">
        <div className="flex items-center gap-3">
          <span className="text-xs font-bold uppercase tracking-widest text-slate-500">Filters</span>
          {hasActive && (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/25 px-3 py-1 text-[10px] font-bold text-cyan-400 tracking-wide">
        ...
        {hasActive && (
          <button
            ...
            className="text-xs text-slate-600 hover:text-rose-400 transition-colors font-medium"
          >
            Reset all
          </button>
        )}
...
function ReportsThesis({ filters }) {
  ...
  const BAR = {
    Thesis:   { bar: "bg-gradient-to-r from-cyan-900 via-cyan-600 to-cyan-400",     val: "text-cyan-400"   },
    Research: { bar: "bg-gradient-to-r from-violet-900 via-violet-600 to-violet-400", val: "text-violet-400" },
    Capstone: { bar: "bg-gradient-to-r from-blue-900 via-blue-600 to-blue-400",     val: "text-blue-400"   },
  };
          ...
          {paginated.map((item, idx) => (
            <tr key={idx} className="hover:bg-slate-800/20 transition-colors group">
              <td className="px-5 py-4 max-w-xs">
                <a href="#" className="text-white hover:text-slate-200 font-semibold text-sm group-hover:underline underline-offset-2 transition-colors line-clamp-1">
                  {item.title}
                </a>
              </td>
              <td className="px-5 py-4 text-slate-500 text-sm">{item.authors}</td>
              <td className="px-5 py-4"><CategoryBadge category={item.category} /></td>
              <td className="px-5 py-4 font-mono text-slate-500 text-xs tabular-nums">{item.year}</td>
              <td className="px-5 py-4 font-mono text-slate-600 text-xs tabular-nums">{item.dateAdded}</td>
            </tr>
          ))}
        </DataTable>
...
function ReportsSimilarity({ filters }) {
  ...
  const simCls    = (v) => v > 50 ? "bg-rose-500/10 text-rose-400 border-rose-500/20" : v > 25 ? "bg-amber-500/10 text-amber-400 border-amber-500/20" : "bg-emerald-500/10 text-emerald-400 border-emerald-500/20";
  const statusCls = (s) => s === "Pending" ? "bg-amber-500/10 text-amber-400 border-amber-500/20" : s === "Reviewed" ? "bg-blue-500/10 text-blue-400 border-blue-500/20" : "bg-emerald-500/10 text-emerald-400 border-emerald-500/20";
        ...
        <div className="space-y-0.5">
          {data.similarityDistribution.map((item, idx) => (
            <ProgressRow
              ...
              colorClass="bg-gradient-to-r from-violet-900 via-violet-600 to-violet-400"
        ...
        <DataTable columns={["Paper Title","Authors","Submitted","Similarity","Status"]}>
          {paginated.map((item, idx) => (
            <tr key={idx} className="hover:bg-slate-800/20 transition-colors group">
              ...
              <td className="px-5 py-4 text-slate-500 text-sm">{item.authors}</td>
              <td className="px-5 py-4 font-mono text-slate-600 text-xs tabular-nums">{item.submissionDate}</td>
...
function ReportsOJT({ filters }) {
  ...
  const colorMap = {
    cyan:    { border:"border-cyan-500/20",    val:"text-cyan-300",    sub:"text-cyan-800",    dot:"bg-cyan-400"    },
    emerald: { border:"border-emerald-500/20", val:"text-emerald-300", sub:"text-emerald-800", dot:"bg-emerald-400" },
    rose:    { border:"border-rose-500/20",    val:"text-rose-300",    sub:"text-rose-800",    dot:"bg-rose-400"    },
    violet:  { border:"border-violet-500/20",  val:"text-violet-300",  sub:"text-violet-800",  dot:"bg-violet-400"  },
  };

  return (
    <div className="space-y-5">
      {/* KPI stat cards — the single biggest visual priority */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {STAT_CARDS.map((s) => {
          const c = colorMap[s.color];
          return (
            <div key={s.label} className={`rounded-2xl border ${c.border} bg-slate-900/40 p-6 hover:bg-slate-900/60 transition-colors`}>
              <div className="flex items-center gap-2 mb-5">
                <span className={`size-2 rounded-full ${c.dot}`} />
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-600">{s.label}</span>
              </div>
              <p className={`text-5xl font-bold tracking-tight leading-none mb-2 ${c.val}`}>{s.value}</p>
      ...
      <Panel>
        ...
        <DataTable columns={["Student Name","Student ID","Academic Year","Coordinator","Progress","Status"]}>
          {paginated.map((item, idx) => {
            ...
            return (
              <tr key={idx} className={`transition-colors ${isOk ? "hover:bg-slate-800/20" : "bg-rose-950/[0.06] hover:bg-rose-950/[0.12]"}`}>
                <td className="px-5 py-4 text-sm font-semibold text-slate-100">{item.studentName}</td>
                <td className="px-5 py-4 font-mono text-slate-600 text-xs tabular-nums">{item.studentId}</td>
                <td className="px-5 py-4">
                  <p className="text-sm text-slate-300 font-medium leading-tight">{item.academicYear}</p>
                  <p className="text-xs text-slate-600 mt-0.5">{item.semester}</p>
                </td>
                ...
                <td className="px-5 py-4">
                  <Badge className={`text-xs font-semibold border rounded-lg px-2.5 py-1 ${isOk ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-rose-500/10 text-rose-400 border-rose-500/20"}`}>
                    {item.overallStatus}
                  </Badge>
                </td>
              </tr>
        ...
        <div className="flex flex-col min-h-screen bg-[#060a10]">
          ...
          <main className="flex-1 px-8 py-10 lg:px-12 lg:py-12">
            <div className="max-w-7xl mx-auto space-y-6">
              ...
              {/* ── PAGE HEADER — largest visual anchor ── */}
              <div className="flex items-end justify-between gap-6 pb-4 border-b border-slate-800/50">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-600 mb-2">Analytics Dashboard</p>
                  <h1 className="text-4xl font-bold text-slate-50 tracking-tight leading-none">Reports</h1>
                </div>

                <div className="flex items-center gap-3 pb-1">
                  {hasActive && (
                    <button onClick={clearAll} className="text-xs text-slate-600 hover:text-rose-400 transition-colors font-medium">
                      Clear filters
                    </button>
                  )}
                  <div className="flex items-center gap-3 bg-slate-900/60 border border-slate-800/60 rounded-xl px-4 py-2.5">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-[9px] font-bold uppercase tracking-widest text-slate-700 leading-none mb-0.5">Live</p>
                    ...
                    <Button size="sm" onClick={() => setLastRefresh(new Date())}
                      className="h-8 px-4 text-xs font-bold bg-gradient-to-r from-cyan-600 to-cyan-500 hover:from-cyan-500 hover:to-cyan-400 text-slate-950 border-0 shadow-[0_2px_12px_rgba(34,211,238,0.25)] hover:shadow-[0_2px_20px_rgba(34,211,238,0.45)] transition-all"
                    >
                      ↻ Refresh
                    </Button>
                  </div>
              ...
              {/* ── REPORT TYPE TABS — second-level hierarchy ── */}
                ...
                {REPORT_TYPES.map((t) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex-1 flex flex-col items-center justify-center gap-1.5 rounded-2xl border py-6 px-4 transition-all duration-200 outline-none cursor-pointer select-none ${
                        active
                          ? "border-slate-700/70 bg-slate-900/60"
                          : "border-slate-800/50 bg-slate-900/20 hover:bg-slate-900/40 hover:border-slate-700/50"
                      }`}
                    >
                      {/* Top accent bar — appears on active */}
                      <div
                      ...
                      <span className={`text-xs font-medium transition-colors ${active ? "text-slate-500" : "text-slate-700"}`}>
                        {t.description}
                      </span>
                    </button>
                  );
              ...
              {hasActive && (
                <div className="flex items-center gap-2.5">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-slate-700">Filtered:</span>
                  <span className="inline-flex items-center gap-2 rounded-full bg-cyan-500/8 border border-cyan-500/20 px-3 py-1 text-xs font-semibold text-cyan-400">
                    <span className="size-1.5 rounded-full bg-cyan-400" />
