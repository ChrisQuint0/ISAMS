...
export function RoleProvider(props) {
  return React.createElement(
    ...
    { value: { role: props.role, studentId: props.studentId } },
    props.children,
  );
}
...
var INITIAL_DOC_FIELDS = [
  ...
  {
    id: "f-4",
    name: "Received Copy of the OJT Recommendation Letter",
    category: "ojt",
    order: 4,
    active: true,
  },
  ...
  {
    id: "f-10",
    name: "Job Proficiency Rating / Evaluation Sheet",
    category: "ojt",
    order: 10,
    active: true,
  },
  ...
  {
    id: "f-15",
    name: "Company Evaluation of Internship",
    category: "hte",
    order: 2,
    active: true,
  },
...
var ALL_FIELD_IDS = INITIAL_DOC_FIELDS.map(function (f) {
  return f.id;
});

function makeUploadedRecord(
  fieldId,
  uploadedBy,
  uploadedAt,
  fileName,
...
var INITIAL_STUDENTS = [
  {
    ...
    uploads: (function () {
      ...
      for (var i = 0; i < uFields.length; i++) {
        r[uFields[i]] = makeUploadedRecord(
          uFields[i],
          "student",
  ...
  },
  {
    ...
    uploads: (function () {
      ...
      for (var i = 0; i < ALL_FIELD_IDS.length; i++) {
        r[ALL_FIELD_IDS[i]] = makeUploadedRecord(
          ALL_FIELD_IDS[i],
          "student",
          "2025-01-10T08:00:00Z",
          "doc_" + ALL_FIELD_IDS[i] + ".pdf",
          400000,
        );
      }
      return r;
...
function getActiveFields(docFields, category) {
  return docFields
    .filter(function (f) {
      return f.category === category && f.active;
    })
    .sort(function (a, b) {
...
function getAllActiveFields(docFields) {
  return docFields.filter(function (f) {
    return f.active;
  });
}
...
function clampOrder(value, length) {
  var parsed = parseInt(value, 10);
  if (isNaN(parsed) || parsed < 1) return 1;
  if (parsed > length) return length;
  return parsed;
}

function buildReorderedCategoryFields(
  docFields,
  category,
  movingFieldId,
  requestedOrder,
  ...
  var movingField =
    insertedField ||
    docFields.find(function (f) {
      return f.id === movingFieldId;
    });
  if (!movingField) return null;

  var categoryFields = docFields
    .filter(function (f) {
      return f.category === category && f.id !== movingField.id;
    })
    .sort(function (a, b) {
      return a.order - b.order;
    });

  var targetIndex = clampOrder(requestedOrder, categoryFields.length + 1) - 1;
  categoryFields.splice(targetIndex, 0, movingField);

  return categoryFields.map(function (field, index) {
    return {
      id: field.id,
      name: field.name,
...
function mergeCategoryFields(docFields, category, reorderedFields) {
  var unchanged = docFields.filter(function (f) {
    return f.category !== category;
  });
  return unchanged.concat(reorderedFields).sort(function (a, b) {
...
// Button classes
var btnBase =
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 outline-none focus-visible:ring-2 focus-visible:ring-primary-500/40 cursor-pointer";
var btnDefault =
  btnBase +
  " h-9 px-4 py-2 bg-primary-500 text-white hover:bg-primary-600 shadow-sm border border-primary-600";
var btnOutline =
  btnBase +
  " h-9 px-4 py-2 border border-neutral-300 bg-white text-neutral-900 shadow-xs hover:bg-neutral-100 hover:border-neutral-400";
var btnGhost =
  btnBase +
  " h-9 px-4 py-2 text-neutral-900 border border-neutral-200 bg-white hover:bg-neutral-100 hover:border-neutral-300 shadow-xs";
var btnSecondary =
  btnBase +
  " h-9 px-4 py-2 bg-neutral-100 text-neutral-900 hover:bg-neutral-200 shadow-xs border border-neutral-300 hover:border-neutral-400";
var btnDestructive =
  btnBase +
  " h-9 px-4 py-2 bg-destructive-semantic text-white hover:bg-destructive-semantic/90 shadow-sm border border-destructive-semantic";
var btnIconGhost =
  btnBase +
  " size-7 text-neutral-500 border border-neutral-200 bg-white hover:bg-neutral-100 hover:text-neutral-900 hover:border-neutral-300 shadow-xs";
var btnIconDestructive =
  btnBase +
  " size-7 text-destructive-semantic border border-destructive-semantic/30 bg-destructive-semantic/5 hover:bg-destructive-semantic/10 hover:border-destructive-semantic/50 shadow-xs";
var btnSmDefault =
  btnBase +
  " h-8 px-3 text-xs gap-1.5 bg-primary-500 text-white hover:bg-primary-600 shadow-sm border border-primary-600";
var btnSmOutline =
  btnBase +
  " h-8 px-3 text-xs gap-1.5 border border-neutral-300 bg-white text-neutral-900 shadow-xs hover:bg-neutral-100 hover:border-neutral-400";
var btnSmGhost =
  btnBase +
  " h-8 px-3 text-xs gap-1.5 text-neutral-500 border border-neutral-200 bg-white hover:bg-neutral-100 hover:text-neutral-900 hover:border-neutral-300 shadow-xs";
var btnSmSecondary =
  btnBase +
  " h-8 px-3 text-xs gap-1.5 bg-neutral-100 text-neutral-900 hover:bg-neutral-200 shadow-xs border border-neutral-300 hover:border-neutral-400";
var btnWarning =
  btnBase +
  " h-9 px-4 py-2 bg-warning text-white hover:bg-warning/90 shadow-sm border border-warning/80";
...
export default function HTEDocumentArchivePage() {
  ...
  const showGoogleAuthModal =
    role === "admin" && googleExpired && !authModalDismissed;
  ...
  var students = s1[0];
  var setStudents = s1[1];
  var s2 = React.useState([]);
  var docFields = s2[0];
  var setDocFields = s2[1];
  var s21 = React.useState({});
  var fieldUsageCounts = s21[0];
  var setFieldUsageCounts = s21[1];
  var s3 = React.useState("");
  var searchQuery = s3[0];
  var setSearchQuery = s3[1];
  var s4 = React.useState("all");
  var yearFilter = s4[0];
  var setYearFilter = s4[1];
  var s5 = React.useState("all");
  var statusFilter = s5[0];
  var setStatusFilter = s5[1];
  var s18 = React.useState("all");
  var programFilter = s18[0];
  var setProgramFilter = s18[1];
  var s19 = React.useState("all");
  var adviserFilter = s19[0];
  var setAdviserFilter = s19[1];
  var s20 = React.useState("all");
  var sectionFilter = s20[0];
  var setSectionFilter = s20[1];
  var [loading, setLoading] = React.useState(true);
  var [isSendingBatch, setIsSendingBatch] = React.useState(false);

  var [filterOptions, setFilterOptions] = React.useState({
    advisers: [],
    sections: [],
    academicYears: [],
  });
  var s6 = React.useState(new Set());
  var selectedStudents = s6[0];
  var setSelectedStudents = s6[1];
  var s7 = React.useState(false);
  var showBatchPreview = s7[0];
  var setShowBatchPreview = s7[1];
  var s8 = React.useState(null);
  var detailStudent = s8[0];
  var setDetailStudent = s8[1];
  var s9 = React.useState(null);
  var uploadError = s9[0];
  var setUploadError = s9[1];
  var s10 = React.useState(null);
  var uploadSuccess = s10[0];
  var setUploadSuccess = s10[1];
  var s15 = React.useState(null);
  var notifySuccess = s15[0];
  var setNotifySuccess = s15[1];
  var s11 = React.useState([]);
  var notificationLog = s11[0];
  var setNotificationLog = s11[1];
  var s12 = React.useState(false);
  var showLog = s12[0];
  var setShowLog = s12[1];
  var s13 = React.useState(false);
  var showFieldConfig = s13[0];
  var setShowFieldConfig = s13[1];
  var up = React.useState(null);
  var uploadingFieldId = up[0];
  var setUploadingFieldId = up[1];
  // Duplicate warn: now carries { blockedStudents, allowedStudents } instead of just a boolean
  var s14 = React.useState(null);
  var duplicateWarnData = s14[0];
  var setDuplicateWarnData = s14[1];
      ...
      const transformed = studentRecords.map((s) => {
        ...
        (s.uploads || []).forEach((u) => {
          uploadsMap[u.field_id] = {
            ...
            file:
              u.status === "uploaded"
                ? { name: u.original_filename, size: u.file_size_bytes }
                : null,
  ...
  async function refreshFieldUsageCounts() {
    try {
      var usageCounts = await thesisService.getHTEDocumentFieldUsage();
      setFieldUsageCounts(usageCounts || {});
    } catch (error) {
      console.error("Failed to refresh field usage counts:", error);
    }
  }
  ...
  var filteredStudents = visibleStudents.filter(function (s) {
    ...
    var matchSearch =
      ...
      (s.student_no || "").toLowerCase().indexOf(sq) !== -1;
    ...
    var matchAdviser =
      adviserFilter === "all" || s.adviser_id === adviserFilter;
  ...
  var incompleteStudents = filteredStudents.filter(function (s) {
    return getStudentStatus(s, docFields) === "incomplete";
  });

  // Incomplete students who are NOT in the cooldown window — these can be selected
  var selectableStudents = incompleteStudents.filter(function (s) {
    return !recentlySentMap[s.id];
  });

  // AG-Grid Column Definitions
  var columnDefs = React.useMemo(
    function () {
      return [
        {
          headerName: "Student",
        ...
        },
        {
          ...
          cellRenderer: function (params) {
            ...
            var ojtPct =
              ojtActive.length === 0
                ? 100
                : Math.round((ojtUploaded / ojtActive.length) * 100);
            var color =
              ojtPct === 100 ? "success" : ojtPct >= 70 ? "primary" : "warning";
            return (
              <div className="flex items-center h-full">
                <ProgressCell
                  uploaded={ojtUploaded}
                  total={ojtActive.length}
                  pct={ojtPct}
                  color={color}
                />
              </div>
        ...
        },
        {
          ...
          cellRenderer: function (params) {
            ...
            var htePct =
              hteActive.length === 0
                ? 100
                : Math.round((hteUploaded / hteActive.length) * 100);
            return (
              <div className="flex items-center h-full">
                <ProgressCell
                  uploaded={hteUploaded}
                  total={hteActive.length}
                  pct={htePct}
                  color="gold"
                />
              </div>
  ...
  async function handleUpload(sId, fieldId, file, uploaderRole) {
    ...
    try {
      const result = await thesisService.uploadHTEDocument(
        sId,
        fieldId,
        file,
        uploaderRole === "admin" ? "coordinator" : "student",
        actorInfo,
      );
      ...
      setStudents(function (prev) {
        return prev.map(function (s) {
          ...
          var newUploads = {};
          Object.keys(s.uploads || {}).forEach(function (k) {
            newUploads[k] = s.uploads[k];
          });
          newUploads[fieldId] = {
            fieldId: fieldId,
            status: "uploaded",
      ...
      if (detailStudent && detailStudent.id === sId) {
        setDetailStudent((prev) => {
          var newUploads = {};
          Object.keys(prev.uploads || {}).forEach(function (k) {
            newUploads[k] = prev.uploads[k];
          });
          newUploads[fieldId] = {
            fieldId: fieldId,
            status: "uploaded",
  ...
  function handleBatchNotifyClick() {
    ...
    var blockedStudentsData = students
      .filter(function (s) {
        return classification.blockedIds.has(s.id);
      })
      .map(function (s) {
        return {
          id: s.id,
          name: s.name,
          email: s.email,
          sentAt: recentlySentMap[s.id],
        };
      });
    var allowedStudentsData = students.filter(function (s) {
      return classification.allowedIds.has(s.id);
    });
  ...
  function handleConfirmAllowedOnly() {
    if (!duplicateWarnData || duplicateWarnData.allowedStudents.length === 0)
      return;
    setDuplicateWarnData(null);
    // Narrow selection to only the allowed students, then open preview
    setSelectedStudents(
      new Set(
        duplicateWarnData.allowedStudents.map(function (s) {
          return s.id;
        }),
      ),
    );
  ...
  async function handleConfirmBatch() {
    ...
    var notified = students.filter(function (s) {
    ...
    try {
      ...
      await thesisService.sendBatchNotifications(batchData, {
        actorName:
          user?.user_metadata?.first_name +
            " " +
            user?.user_metadata?.last_name || "OJT Coordinator",
        actorUserId: user?.id,
      });
      ...
      setNotificationLog(function (prev) {
  ...
  async function handleAddField(name, category, order) {
    try {
      var categoryCount = docFields.filter(function (f) {
        return f.category === category;
      }).length;
      var normalizedRequestedOrder = clampOrder(order, categoryCount + 1);

      var created = await thesisService.createHTEDocumentField({
        name: name,
        category: category,
        display_order: normalizedRequestedOrder,
      });
      ...
      var reordered = buildReorderedCategoryFields(
        docFields,
        category,
        insertedField.id,
        normalizedRequestedOrder,
        insertedField,
      );

      if (reordered) {
        await thesisService.bulkUpdateHTEDocumentFieldOrders(
          reordered.map(function (field) {
            return { id: field.id, display_order: field.order };
          }),
        );

        setDocFields(function (prev) {
          return mergeCategoryFields(
            prev.concat([insertedField]),
            category,
            reordered,
          );
        });
      ...
      setStudents(function (prev) {
        return prev.map(function (s) {
          if (s.uploads[insertedField.id]) return s;
          var newUploads = Object.assign({}, s.uploads);
          newUploads[insertedField.id] = {
          ...
          return Object.assign({}, s, { uploads: newUploads });
        });
      });

      setUploadSuccess(
        '"' + name + '" has been added as a new document field.',
      );
      setTimeout(function () {
        setUploadSuccess(null);
      }, 4000);
    } catch (error) {
      console.error("Failed to add document field:", error);
      setUploadError(
        "Failed to add document field: " + (error.message || "Unknown error"),
      );
      setTimeout(function () {
        setUploadError(null);
      }, 5000);
  ...
  async function handleToggleFieldActive(fieldId) {
    var current = docFields.find(function (f) {
      return f.id === fieldId;
    });
    if (!current) return;
    ...
    try {
      await thesisService.updateHTEDocumentField(fieldId, {
      ...
      console.error("Failed to update field active state:", error);
      setUploadError(
        "Failed to update field status: " + (error.message || "Unknown error"),
      );
      setTimeout(function () {
        setUploadError(null);
      }, 5000);
  ...
  async function handleDragReorderField(draggedFieldId, targetFieldId) {
    var dragged = docFields.find(function (f) {
      return f.id === draggedFieldId;
    });
    var target = docFields.find(function (f) {
      return f.id === targetFieldId;
    });
    if (!dragged || !target) return;
    if (dragged.id === target.id || dragged.category !== target.category)
      return;
    ...
    try {
      ...
      setDocFields(function (prev) {
        return mergeCategoryFields(prev, dragged.category, reordered);
      });
    } catch (error) {
      console.error("Failed to reorder fields by drag:", error);
      setUploadError(
        "Failed to reorder fields: " + (error.message || "Unknown error"),
      );
      setTimeout(function () {
        setUploadError(null);
      }, 5000);
  ...
  async function handleUpdateFieldName(fieldId, newName) {
    try {
      await thesisService.updateHTEDocumentField(fieldId, { name: newName });
      setDocFields(function (prev) {
        return prev.map(function (f) {
          return f.id !== fieldId
            ? f
      ...
      setUploadError(
        "Failed to update field name: " + (error.message || "Unknown error"),
      );
      setTimeout(function () {
        setUploadError(null);
      }, 5000);
  ...
  async function handleDeleteField(fieldId) {
    var usageCount = fieldUsageCounts[fieldId] || 0;
    if (usageCount > 0) return;

    var fieldToDelete = docFields.find(function (f) {
      return f.id === fieldId;
    });
    if (!fieldToDelete) return;

    try {
      await thesisService.deleteHTEDocumentField(fieldId);

      var remainingCategory = docFields
        .filter(function (f) {
          return f.category === fieldToDelete.category && f.id !== fieldId;
        })
        .sort(function (a, b) {
          return a.order - b.order;
        })
      ...
      if (remainingCategory.length > 0) {
        await thesisService.bulkUpdateHTEDocumentFieldOrders(
          remainingCategory.map(function (field) {
            return { id: field.id, display_order: field.order };
          }),
        );
      }
      ...
      setDocFields(function (prev) {
        var withoutDeleted = prev.filter(function (f) {
          return f.id !== fieldId;
        });
        return mergeCategoryFields(
          withoutDeleted,
      ...
      setFieldUsageCounts(function (prev) {
        var next = Object.assign({}, prev);
        delete next[fieldId];
        return next;
      });

      setUploadSuccess("Document field deleted.");
      setTimeout(function () {
        setUploadSuccess(null);
      }, 3500);
    } catch (error) {
      ...
      setUploadError(
        "Failed to delete field: " + (error.message || "Unknown error"),
      );
      setTimeout(function () {
        setUploadError(null);
      }, 5000);
  ...
  var selectedNotified = students.filter(function (s) {
    return selectedStudents.has(s.id);
  });
  var liveDetail = detailStudent
    ? students.find(function (s) {
        return s.id === detailStudent.id;
      })
    : null;
    ...
    <div className="flex flex-col min-h-screen w-full bg-neutral-100 text-neutral-900">
        ...
        <div className="space-y-5">
          {role === "student" ? (
            <HTEStudentPage
              ...
              docFields={docFields}
              onUpload={function (sId, fieldId, file) {
                handleUpload(sId, fieldId, file, "student");
              }}
            />
          ) : (
            <React.Fragment>
              {role === "admin" && showFieldConfig && (
                <FieldConfigPanel
                  ...
                  onDragReorderField={handleDragReorderField}
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <SelectInput
                          ...
                          options={[
                            ...
                            filterOptions.academicYears.map(function (y) {
                        ...
                        <SelectInput
                          ...
                          options={[
                            ...
                            filterOptions.advisers.map(function (a) {
                              return {
                                value: a.id,
                                label:
                                  a.display_name ||
                        ...
                        <SelectInput
                          ...
                          options={[
                            ...
                            Array.from(
                              new Set(
                                filterOptions.sections.map(function (s) {
                                  return s.name;
                                }),
                              ),
                            ).map(function (name) {
                  ...
                  {role === "admin" && selectedStudents.size > 0 && (
                    <div className="bg-neutral-100 border border-neutral-200 rounded-xl p-4 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-neutral-500">
                            Ready to send batch notification
                          </p>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={handleBatchNotifyClick}
                  ...
                  <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-sm">
                    <div className="px-5 py-4 border-b border-neutral-200 flex items-center justify-between bg-gradient-primary">
                      <div className="flex items-center gap-3">
                        ...
                        {role === "admin" && (
                          <span className="text-xs px-2 py-0.5 rounded-full bg-white/15 border border-white/25 text-white/80">
                      ...
                      {role === "admin" && selectedStudents.size > 0 && (
                        <button
                          ...
                          className="inline-flex items-center gap-2 text-xs font-medium px-4 py-2 rounded-lg bg-primary-600 text-white hover:bg-primary-700 shadow-md shadow-primary-500/20 transition-all"
                        >
                    ...
                    <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-sm">
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-xs px-2 py-0.5 rounded-full border border-neutral-200 bg-neutral-100 text-neutral-500">
                            {notificationLog.length} event
                            ...
                            return (
                              <div key={entry.id} className="p-5">
                                <div className="flex items-start justify-between mb-2">
                                  ...
                                  <span className="text-xs px-2 py-0.5 rounded-full bg-neutral-100 border border-neutral-200 text-neutral-500">
                                    {entry.id}
                                  </span>
                                </div>
                                <div className="ml-11 flex flex-wrap gap-1.5">
                                  {entry.students.map(function (s) {
                                    return (
                                      <span
                                        key={s.id}
                                        className="text-xs bg-neutral-100 border border-neutral-200 text-neutral-900 px-2 py-0.5 rounded-full"
                                      >
                                        {s.name}
                                      </span>
                                    );
        ...
        >
          <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <p className="text-xs text-neutral-500 mt-0.5">
                  {duplicateWarnData.blockedStudents.length === 1
                    ? "1 student was"
                    : duplicateWarnData.blockedStudents.length +
                      " students were"}{" "}
                  already notified within the {DUPLICATE_SEND_COOLDOWN_MINUTES}
                  -minute cooldown window.
                </p>
              </div>
            </div>
            ...
            {duplicateWarnData.allowedStudents.length > 0 ? (
              <p className="text-sm text-neutral-600">
                <span className="font-semibold text-neutral-900">
                  {duplicateWarnData.allowedStudents.length} other student
                  {duplicateWarnData.allowedStudents.length !== 1 ? "s" : ""}
                </span>{" "}
                in your selection are not in cooldown and can be notified now.
              </p>
            ) : (
              <p className="text-sm text-neutral-600">
                All selected students are currently in the cooldown window.
                Sending duplicate notifications may confuse them.
              </p>
            )}
            ...
            <div className="flex flex-col gap-2 pt-1">
              ...
              {duplicateWarnData.allowedStudents.length > 0 ? (
                <button
                  onClick={handleConfirmAllowedOnly}
                  className={btnDefault + " w-full justify-center"}
                >
                  <Mail className="h-4 w-4" />
                  <span>
              ...
              <button
                onClick={handleForceConfirmAll}
                className={btnWarning + " w-full justify-center"}
              >
                <ShieldAlert className="h-4 w-4" />
                <span>
                  Override Cooldown & Notify All {selectedStudents.size}
                </span>
              </button>
              <button
                onClick={function () {
          ...
          <div className="flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-neutral-200 bg-gradient-primary">
              <h2 className="text-xl font-bold text-white">
                Preview Batch Notification
              </h2>
              <p className="text-sm text-white/70">
                Emails will be sent to{" "}
                <span className="text-gold-500 font-semibold">
                  {selectedNotified.length} student(s)
                </span>
                  ...
                  >
                    <div className="px-4 py-2.5 bg-neutral-100 border-b border-neutral-200 flex items-center justify-between gap-3 flex-wrap">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <span className="text-xs px-2 py-0.5 rounded-full bg-warning/10 border border-warning/40 text-warning font-medium">
                    ...
                    <div className="px-4 py-2 bg-neutral-50 border-b border-neutral-200 flex items-center gap-3 text-xs">
                      ...
                      <span className="text-neutral-900">
                        Action Required: Pending OJT Document Submissions —{" "}
                        {student.year}
                      </span>
                    </div>
                    <div className="p-5 space-y-4 text-sm text-neutral-900 leading-relaxed">
                      <p>
                        Dear <span className="font-bold">{student.name}</span>,
                      </p>
                      <p className="text-neutral-500">
                        This is a reminder from your OJT Coordinator regarding
                        your document submissions for the{" "}
                        <span className="text-neutral-900 font-medium">
                          {student.year}
                        </span>{" "}
                        internship period. The following document(s) are still
                        pending:
                      </p>
                      ...
                      <p className="text-neutral-500 font-medium">
                        Please upload your missing documents through the ISAMS
                        Desktop App.
                      </p>
                      <p className="text-neutral-500 text-xs pt-1 border-t border-neutral-100">
                        If you have already submitted any documents in person,
                        please coordinate with your OJT Coordinator for manual
                        processing. Do not reply to this automated message.
                      </p>
                    </div>
                  </div>
                );
              ...
              >
                {isSendingBatch ? (
                  ...
                  <React.Fragment>
                    <Mail className="h-4 w-4" />
                    <span>
                      Confirm and Send {selectedNotified.length} Email(s)
                    </span>
                  </React.Fragment>
                )}
              </button>
      ...
      {detailStudent ? (
        <StudentDetailModal
          ...
          onUpload={function (fieldId, file) {
            handleUpload(detailStudent.id, fieldId, file, role);
          }}
          onRemoveUpload={function (fieldId) {
            handleRemoveUpload(detailStudent.id, fieldId);
          }}
      ...
      {/* Toast stack */}
      <div
        className="fixed left-1/2 -translate-x-1/2 z-60 flex flex-col gap-3 w-96 pointer-events-none"
        style={{ top: "calc(var(--titlebar-height) + 12px)" }}
      >
        {uploadError ? (
          <Toast
            type="error"
...
function GoogleAuthButton(props) {
  ...
  var checkStatus = React.useCallback(
    async function (force) {
      if (!userId) return;
      // Prevent concurrent or redundant checks
      if (!force && isFetching.current) return;
      if (!force && lastCheckedId.current === userId && authUrlRef.current)
        return;
      ...
      try {
        ...
        console.log("GoogleAuthButton: Done", {
          authenticated: res.authenticated,
          hasUrl: !!url,
        });
      } catch (error) {
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          {status.authenticated ? (
            <button
              className={
                btnSmSecondary +
                " relative group border-primary-500/30 bg-primary-500/5 hover:bg-primary-500/10"
              }
            >
              <img
                ...
                alt="Google"
              />
              <span className="text-[11px] font-semibold text-primary-700 ml-1">
                Connected
              </span>
        ...
        <TooltipContent side="bottom" className="text-[10px] p-2 max-w-[200px]">
          {status.isLoading ? (
            ...
            <div>
              <p className="font-bold text-primary-600">Connected</p>
              <p className="truncate">{status.email}</p>
            </div>
          ) : (
            <div>
              ...
              <p>
                {authUrl
                  ? "Click to authenticate your Google Drive to enable uploads"
                  : "Preparing authentication link..."}
              </p>
            </div>
          )}
        </TooltipContent>
        ...
        <div className="flex items-center gap-3">
          ...
          {role === "admin" && (
            <button
              ...
              className={btnDefault + " h-8 px-3 text-xs"}
            >
...
function FieldConfigPanel(props) {
  ...
  var fieldUsageCounts = props.fieldUsageCounts || {};
  var onDragReorderField = props.onDragReorderField;
  var onDeleteField = props.onDeleteField;
  var fa = React.useState("");
  var newName = fa[0];
  var setNewName = fa[1];
  var fb = React.useState("ojt");
  var newCategory = fb[0];
  var setNewCategory = fb[1];
  var fc = React.useState("1");
  var newOrder = fc[0];
  var setNewOrder = fc[1];
  var fd = React.useState(null);
  var addError = fd[0];
  var setAddError = fd[1];
  var fe = React.useState(null);
  var draggingFieldId = fe[0];
  var setDraggingFieldId = fe[1];
  ...
  async function handleAdd() {
    ...
    if (isNaN(parsed) || parsed < 1) {
      setAddError("Display order must be a positive integer.");
      return;
    }
    setAddError(null);
    try {
      await onAddField(newName.trim(), newCategory, parsed);
      setNewName("");
      setNewOrder("1");
    } catch (error) {
      setAddError(error.message || "Failed to add field.");
    }
  }
  ...
  React.useEffect(
    function () {
      ...
      function handleMouseUp() {
        if (dragOverFieldId && dragOverFieldId !== draggingFieldId) {
          onDragReorderField(draggingFieldId, dragOverFieldId);
        }
    ...
    [draggingFieldId, dragOverFieldId, onDragReorderField],
  );

  function renderFieldRow(field) {
    var canDelete = (fieldUsageCounts[field.id] || 0) === 0;
    var isDragging = draggingFieldId === field.id;
    var isDropTarget =
    ...
    return (
      <div
        ...
        onMouseEnter={function () {
          ...
          var draggedField = docFields.find(function (f) {
            return f.id === draggingFieldId;
          });
          if (!draggedField || draggedField.category !== field.category) return;
        ...
        className={
          ...
          (isDragging
            ? " ring-2 ring-primary-500/50 border-primary-500/60 bg-primary-500/5 shadow-sm scale-[1.01]"
            : "") +
          (isDropTarget
            ? " ring-2 ring-primary-500/40 border-primary-500/50 bg-primary-500/10"
            : "")
        }
      >
        <button
          ...
          className={
            ...
            (isDragging ? "bg-primary-500/10 border-primary-500/30" : "")
          }
        >
        ...
        <input
          ...
          readOnly
          draggable={false}
          className="w-14 px-2 py-1 bg-neutral-50 border border-neutral-200 rounded text-neutral-900 text-xs text-center cursor-default"
        />
        <input
          type="text"
          value={field.name}
          draggable={false}
          onChange={function (e) {
            onUpdateName(field.id, e.target.value);
          }}
        ...
        {canDelete ? (
          <button
            ...
            title="Delete unused field"
            className={btnIconDestructive}
          >
        ...
        <button
          ...
          className={
            ...
            " " +
            (field.active
              ? "text-success hover:text-destructive-semantic hover:bg-destructive-semantic/10"
  ...
  return (
    <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-sm">
      <div className="px-5 py-4 border-b border-neutral-200 flex items-center gap-3 bg-gradient-primary">
        ...
        <span className="text-xs px-2 py-0.5 rounded-full border border-gold-500/50 bg-gold-500/15 text-gold-500">
          Admin Only
        </span>
      </div>
      <div className="p-6 space-y-6">
        {draggingFieldId ? (
          <div className="text-xs text-primary-600 bg-primary-500/10 border border-primary-500/20 rounded-lg px-3 py-2">
            Dragging field. Release on another row to reposition.
          </div>
        ) : null}
        <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-5">
          ...
          {addError ? (
            <p className="text-xs text-destructive-semantic mt-2">{addError}</p>
          ) : null}
          <div className="mt-4 flex justify-end">
            <button onClick={handleAdd} className={btnDefault}>
...
function StudentDetailModal(props) {
  ...
  var selectedTerm = termArr[0];
  var setSelectedTerm = termArr[1];
  var availableTerms = [
    s.year + " " + s.semester,
      ...
      <div className="flex flex-col max-h-[90vh]">
        <div className="p-6 border-b border-neutral-200 bg-gradient-primary">
          ...
          <p className="text-sm text-white/70 mt-1">
            {s.name} <span className="text-gold-500">|</span> {s.id}{" "}
            <span className="text-gold-500">|</span> {s.year} {s.semester}
          </p>
        </div>
...
function DocSection(props) {
  ...
  var showInactive = props.showInactive;
  var termSelector = props.termSelector;
  var hteLocked = props.hteLocked;
  var uploadingFieldId = props.uploadingFieldId;
  var badgeClass =
    badgeColor === "primary"
      ? "bg-primary-500/8 border-primary-500/20 text-primary-500"
    ...
    <div>
      <div className="flex items-start justify-between mb-3 gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          {hteLocked ? (
            <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border border-neutral-200 bg-neutral-100 text-neutral-500">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          >
            {uploadedCount} / {activeCount} uploaded
          </span>
        </div>
      </div>
      <div className="space-y-2">
        {fields.map(function (field) {
          ...
          return (
            <DocumentItem
              ...
              canDownload={canDownload && !isInactive}
...
function DocumentItem(props) {
  ...
  var expandedArr = React.useState(false);
  var expanded = expandedArr[0];
  var setExpanded = expandedArr[1];
  ...
  async function handleDownload() {
    ...
    try {
      await thesisService.downloadHTEDocument(
        rec.gdrive_file_id,
        rec.file?.name,
      );
      ...
      <div className="p-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {!isInactive && canUpload ? (
              <span>
                ...
                <button
                  ...
                  title={
                    status === "uploaded" ? "Replace file" : "Upload document"
                  }
                  onClick={function () {
            ...
            {status === "uploaded" && canRemove && !isUploading ? (
              <button
                title="Remove file"
                onClick={function () {
                  onRemoveUpload(field.id);
                }}
                className={btnIconDestructive}
              >
        ...
        {expanded && status === "uploaded" && rec ? (
          <div className="mt-3 pt-3 border-t border-neutral-100 grid grid-cols-2 gap-3 text-xs">
            <div className="flex items-center gap-2 text-neutral-500">
              <FileText className="h-3.5 w-3.5 text-neutral-200" />
              <span className="text-neutral-900 truncate">
                {rec.file && rec.file.name ? rec.file.name : "-"}
              </span>
            </div>
            <div className="flex items-center gap-2 text-neutral-500">
              <User className="h-3.5 w-3.5 text-neutral-200" />
              <span>
                {rec.uploadedBy === "coordinator" ? "Coordinator" : "Student"}
              </span>
            </div>
            <div className="flex items-center gap-2 text-neutral-500">
...
function StatusBadge(props) {
  ...
  return (
    <span
      className={
        "inline-flex items-center gap-1 text-xs px-2.5 py-0.5 rounded-full border font-semibold " +
        (isComplete
          ? "bg-success/10 border-success/30 text-success"
          : "bg-neutral-100 border-neutral-200 text-neutral-500")
      }
...
function Modal(props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        className={
          "relative bg-white border border-neutral-200 rounded-2xl shadow-2xl w-full overflow-hidden " +
...
function Toast(props) {
  ...
  var config = {
    error: {
      icon: AlertTriangle,
      iconCls: "text-destructive-semantic",
      bar: "bg-destructive-semantic",
      bg: "bg-white",
      border: "border-destructive-semantic/20",
      title: "Error",
    },
    success: {
      icon: CheckCircle2,
  ...
  return (
    <div
      className={
        "pointer-events-auto flex items-stretch rounded-xl border shadow-lg overflow-hidden " +
        c.bg +
        " " +
        c.border
      }
    >
      ...
      <div className="flex items-start gap-3 px-4 py-3.5 flex-1 min-w-0">
        <div className={"mt-0.5 flex-shrink-0 " + c.iconCls}>
        ...
        <button
          onClick={onClose}
          className={btnIconGhost + " -mr-1 flex-shrink-0"}
        >
          <X className="h-4 w-4" />
        </button>
      </div>
