...
export default function StudentDocumentPortal() {
  ...
  async function loadStudentData(userId) {
    ...
    try {
      ...
      if (!studentRow) {
        setDataError(
          "No student record found for your account. Please contact your coordinator.",
        );
        return;
      }
...
const btnIconDestructive =
  btnBase +
  " size-7 text-red-500 border border-red-200/50 bg-red-50 hover:bg-red-100 hover:border-red-300 shadow-xs";
const btnSmDefault =
  btnBase +
...
function isOtherCategory(category) {
  return category !== "ojt" && category !== "hte";
}

function getOtherCategoryLabel(fields) {
  ...
  if (names.length <= 1) return "Additional Documents";
  return "Additional Document Categories";
}
...
function StudentDocumentPortalContent(props) {
  ...
  var pa = React.useState(PAGE.OVERVIEW);
  var activePage = pa[0];
  var setActivePage = pa[1];
  var ta = React.useState(null);
  var toast = ta[0];
  var setToast = ta[1];
  ...
  function handleUploadSuccess(name) {
    setToast({ type: "success", msg: '"' + name + '" uploaded successfully.' });
    setTimeout(function () {
      setToast(null);
    }, 3500);
  }
  ...
  function handleRemoveWrap(fieldId) {
    ...
    setToast({ type: "success", msg: "Document removed successfully." });
    setTimeout(function () {
      setToast(null);
    }, 3500);
  }
  ...
  var otherFields = docFields
    .filter(function (f) {
      return isOtherCategory(f.category);
    })
    .sort(function (a, b) {
  ...
  var ojtActive = ojtFields.filter(function (f) {
    return f.active;
  });
  var hteActive = hteFields.filter(function (f) {
    return f.active;
  });
  var otherActive = otherFields.filter(function (f) {
    return f.active;
  });

  var ojtUploaded = ojtActive.filter(function (f) {
    var r = student.uploads[f.id];
    return resolveStatus(f, r) === "uploaded";
  }).length;
  var hteUploaded = hteActive.filter(function (f) {
    var r = student.uploads[f.id];
    return resolveStatus(f, r) === "uploaded";
  }).length;
  var otherUploaded = otherActive.filter(function (f) {
    var r = student.uploads[f.id];
    return resolveStatus(f, r) === "uploaded";
  }).length;
  ...
  var otherPct =
    otherActive.length === 0
      ? 100
      : Math.round((otherUploaded / otherActive.length) * 100);
  var totalAct = ojtActive.length + hteActive.length + otherActive.length;
  var totalUp = ojtUploaded + hteUploaded + otherUploaded;
  var totalPct = totalAct === 0 ? 100 : Math.round((totalUp / totalAct) * 100);
  var isComplete = totalPct === 100;
  var otherLabel = getOtherCategoryLabel(otherFields);
  ...
  return (
    <div className="flex min-h-screen w-full bg-neutral-100 text-neutral-900">
      <StudentSidebar
        ...
        student={student}
        onSignOut={onSignOut}
        hasOtherPage={otherFields.length > 0}
        ...
        <main className="flex-1 w-full p-6">
          {activePage === PAGE.OVERVIEW && (
            <OverviewPage
              ...
              ojtUploaded={ojtUploaded}
              hteUploaded={hteUploaded}
              otherUploaded={otherUploaded}
              ojtPct={ojtPct}
              htePct={htePct}
              otherPct={otherPct}
              totalPct={totalPct}
          ...
          {activePage === PAGE.OTHER && otherFields.length > 0 && (
            <OtherDocumentsPage
              student={student}
              fields={otherFields}
              uploaded={otherUploaded}
              total={otherActive.length}
              pct={otherPct}
              onUpload={handleUpload}
              onRemove={handleRemoveWrap}
              uploadingFieldId={uploadingFieldId}
...
function StudentSidebar(props) {
  ...
  var student = props.student;
  var onSignOut = props.onSignOut;
  var hasOtherPage = props.hasOtherPage;
  var otherLabel = props.otherLabel;

  var cpState = React.useState(false);
  var showChangePassword = cpState[0];
  var setShowChangePassword = cpState[1];
  var modalState = React.useState(false);
  var showPasswordModal = modalState[0];
  var setShowPasswordModal = modalState[1];
  ...
  var navItems = [
    ...
    hasOtherPage
      ? {
          page: PAGE.OTHER,
          icon: Upload,
          label: otherLabel || "Additional Documents",
        }
      : null,
  ...
  return (
    <aside
      className={
        "flex-shrink-0 flex flex-col min-h-screen sticky top-0 h-screen bg-white border-r border-neutral-200 transition-[width] duration-200 overflow-hidden " +
    ...
    >
      ...
      <div
        className={
          "h-14 border-b border-neutral-200 flex items-center flex-shrink-0 " +
      ...
      <nav
        className={
          "flex-1 overflow-y-auto " + (isCollapsed ? "px-2 py-3" : "p-3")
        }
      >
        {!isCollapsed && (
          <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider px-2 pt-1 pb-2">
            Menu
          </p>
        )}
      ...
      <div
        className={
          "border-t border-neutral-200 flex-shrink-0 " +
        ...
        {isCollapsed ? (
          <div className="flex flex-col items-center gap-2">
            <button
              ...
              className="h-8 w-8 rounded-full bg-primary-500/10 border border-primary-500/20 flex items-center justify-center cursor-pointer hover:bg-primary-500/20 transition-all"
            >
            ...
            <button
              onClick={onSignOut}
              title="Sign Out"
              className="h-8 w-8 flex items-center justify-center rounded-lg text-destructive-semantic hover:bg-destructive-semantic/10 transition-all cursor-pointer"
            >
          ...
          <div className="space-y-2">
            ...
            <div
              onClick={function () {
                setShowChangePassword(function (p) {
                  return !p;
                });
              }}
              className={
                "bg-neutral-50 border rounded-xl p-3 space-y-2.5 cursor-pointer transition-all " +
                (showChangePassword
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <ChevronRight
                  className={
                    "h-3 w-3 text-neutral-400 flex-shrink-0 transition-transform duration-200 " +
              ...
              <div className="space-y-1 text-[10px] text-neutral-500 pl-0.5">
                <div className="flex items-center gap-1.5">
                  <Calendar className="h-2.5 w-2.5 text-neutral-400 flex-shrink-0" />
                  <span>
                    {student.year} · {student.semester}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  <GraduationCap className="h-2.5 w-2.5 text-neutral-400 flex-shrink-0" />
            ...
            <button
              onClick={onSignOut}
              className="w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-sm font-medium text-destructive-semantic bg-destructive-semantic/8 border border-destructive-semantic/20 hover:bg-destructive-semantic/15 hover:border-destructive-semantic/30 transition-all cursor-pointer"
            >
              <LogOut className="h-4 w-4" />
              <span>Sign Out</span>
            </button>
...
function OverviewPage(props) {
  ...
  var otherFields = props.otherFields;
  var otherActive = props.otherActive;
  var ojtUploaded = props.ojtUploaded;
  var hteUploaded = props.hteUploaded;
  var otherUploaded = props.otherUploaded;
  var ojtPct = props.ojtPct;
  var htePct = props.htePct;
  var otherPct = props.otherPct;
    ...
    <div className="w-full space-y-5">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p
            className={
              "text-sm mt-1 " +
              (isComplete ? "text-success/80" : "text-white/75")
            }
          >
            {isComplete
              ? "Your submission is complete. Your coordinator will review your documents."
              : pendingOjt.length +
                " document(s) still pending — upload them to complete your submission."}
          </p>
          <p
            className={
              "text-xs mt-1 " +
              (isComplete ? "text-success/60" : "text-white/50")
            }
          >
            {student.year} · {student.semester} · {student.id}
          </p>
        </div>
        <div className="text-right flex-shrink-0">
          ...
          <p
            className={
              "text-xs mt-1 " +
              (isComplete ? "text-success/70" : "text-white/60")
            }
          >
            {totalUp} / {totalAct} docs
          </p>
        </div>
      </div>
      ...
      <div
        className={
          ...
          (otherFields.length > 0 ? "lg:grid-cols-3" : "")
        }
      >
        <button
          onClick={function () {
          ...
          className="group text-left bg-white border border-neutral-200 rounded-xl p-5 shadow-sm hover:shadow-md hover:border-primary-500/30 transition-all"
        >
          <div className="flex items-start justify-between mb-4">
            ...
            <span
              className={
                "text-3xl font-black " +
                (ojtPct === 100 ? "text-success" : "text-primary-500")
              }
            >
              {ojtPct}%
            </span>
          ...
          <div className="w-full h-2 bg-neutral-100 rounded-full overflow-hidden mb-2">
            <div
              className={
                ...
                (ojtPct === 100 ? "bg-success" : "bg-primary-500")
              }
              style={{ width: ojtPct + "%" }}
            />
          </div>
          <div className="flex items-center justify-between">
            <span className="text-xs text-neutral-500">
              {ojtUploaded} of {ojtActive.length} uploaded
            </span>
            <span className="text-xs text-primary-500 font-semibold flex items-center gap-1 group-hover:gap-1.5 transition-all">
              Upload docs <ChevronRight className="h-3 w-3" />
            </span>
          </div>
        </button>
        <button
          ...
          className="group text-left bg-white border border-neutral-200 rounded-xl p-5 shadow-sm hover:shadow-md hover:border-neutral-300 transition-all"
        >
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-xl bg-neutral-100 border border-neutral-200 flex items-center justify-center">
            ...
            <span
              className={
                "text-3xl font-black " +
                (htePct === 100 ? "text-success" : "text-neutral-400")
              }
            >
              {htePct}%
            </span>
          ...
          <div className="w-full h-2 bg-neutral-100 rounded-full overflow-hidden mb-2">
            <div
              className={
                "h-full rounded-full transition-all duration-700 " +
                (htePct === 100 ? "bg-success" : "bg-neutral-400")
              }
              style={{ width: htePct + "%" }}
            />
          </div>
          <div className="flex items-center justify-between">
            <span className="text-xs text-neutral-500">
              {hteUploaded} of {hteActive.length} uploaded
            </span>
            <span className="text-xs text-neutral-500 font-semibold flex items-center gap-1 group-hover:gap-1.5 transition-all">
          ...
          >
            <div className="flex items-start justify-between mb-4">
              ...
              <span
                className={
                  "text-3xl font-black " +
                  (otherPct === 100 ? "text-success" : "text-neutral-400")
                }
              >
                {otherPct}%
              </span>
            ...
            <div className="w-full h-2 bg-neutral-100 rounded-full overflow-hidden mb-2">
              <div
                className={
                  "h-full rounded-full transition-all duration-700 " +
                  (otherPct === 100 ? "bg-success" : "bg-neutral-400")
                }
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs text-neutral-500">
                {otherUploaded} of {otherActive.length} uploaded
              </span>
              <span className="text-xs text-neutral-500 font-semibold flex items-center gap-1 group-hover:gap-1.5 transition-all">
      ...
      {pendingOjt.length > 0 && (
        <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-sm">
          <div className="px-5 py-4 border-b border-neutral-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="h-8 w-8 rounded-lg bg-warning/10 border border-warning/30 flex items-center justify-center">
                <Clock className="h-4 w-4 text-warning" />
              </div>
              <div>
                <p className="text-sm font-bold text-neutral-900">
                  Pending OJT Documents
                </p>
                <p className="text-xs text-neutral-500">
                  {pendingOjt.length} document
                  {pendingOjt.length !== 1 ? "s" : ""} awaiting upload
                </p>
              </div>
            </div>
            <button
          ...
          <div className="divide-y divide-neutral-50">
            ...
            {pendingOjt.length > 6 && (
              <button
                ...
                className="w-full px-5 py-3 text-left text-xs text-neutral-500 hover:text-primary-500 hover:bg-neutral-50 transition-colors"
              >
      ...
      <div className="bg-primary-500/5 border border-primary-500/20 rounded-xl p-4 flex items-start gap-3">
        ...
        <div className="text-xs text-neutral-600 leading-relaxed space-y-1">
          <p>
            ...
            — 13 required fields. Upload PDF, DOCX, JPG, or PNG files up to 10
            MB each.
          </p>
          <p>
            <span className="font-semibold text-neutral-900">
            ...
            — Company Profile and Company Evaluation. Upload PDF, DOCX, JPG, or
            PNG files up to 10 MB each.
          </p>
          {otherFields.length > 0 && (
            <p>
              <span className="font-semibold text-neutral-900">
              ...
              — Coordinator-defined custom fields. Upload PDF, DOCX, JPG, or PNG
              files up to 10 MB each.
            </p>
          )}
          <p>
            <span className="font-semibold text-neutral-900">Not Required</span>{" "}
            — Fields deactivated by your coordinator. Shown for visibility but
...
function OtherDocumentsPage(props) {
  var student = props.student;
  ...
  var title = props.title || "Additional Documents";
  var isComplete = pct === 100;

  return (
    <div className="w-full space-y-5">
      <div className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm flex items-center gap-4">
        <div className="h-12 w-12 rounded-xl bg-neutral-100 border border-neutral-200 flex items-center justify-center flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-neutral-500 mt-1">
            Coordinator-configured document requirements
          </p>
        </div>
        <div className="text-right flex-shrink-0">
          <p
            className={
              ...
              (isComplete ? "text-success" : "text-neutral-400")
            }
          >
            {pct}%
          </p>
      ...
      <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-sm">
        <div className="px-5 py-3.5 border-b border-neutral-200 bg-neutral-800 flex items-center justify-between">
          ...
          <span className="text-xs px-2.5 py-1 rounded-full bg-white/15 border border-white/20 text-white/90 font-semibold">
            {uploaded} / {total} uploaded
          </span>
        </div>
...
function OJTPage(props) {
  ...
  return (
    <div className="w-full space-y-5">
      <div className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm flex items-center gap-4">
        <div className="h-12 w-12 rounded-xl bg-primary-500/10 border border-primary-500/20 flex items-center justify-center flex-shrink-0">
        ...
        <div className="text-right flex-shrink-0">
          <p
            className={
              "text-3xl font-black leading-none " +
              (isComplete ? "text-success" : "text-primary-500")
            }
          >
            {pct}%
          </p>
      ...
      <div className="bg-warning/8 border border-warning/30 rounded-xl p-4 flex items-start gap-3">
        ...
        <p className="text-xs text-neutral-700 leading-relaxed">
          Upload all required OJT Trainee documents below. Accepted formats:{" "}
          <span className="font-semibold text-neutral-900">
            PDF, DOCX, JPG, PNG
          </span>{" "}
          · Max size:{" "}
          <span className="font-semibold text-neutral-900">10 MB</span>. Slots
          marked{" "}
          <span className="font-semibold text-neutral-900">Not Required</span>{" "}
          have been deactivated by your coordinator and do not count toward
          completion.
        </p>
      </div>
      ...
      <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-sm">
        <div className="px-5 py-3.5 border-b border-neutral-200 bg-gradient-primary flex items-center justify-between">
          ...
          <span className="text-xs px-2.5 py-1 rounded-full bg-white/15 border border-white/25 text-white font-semibold">
            {uploaded} / {total} uploaded
          </span>
        </div>
        <div className="divide-y divide-neutral-100">
...
function OJTDocRow(props) {
  ...
  return (
    <div
      className={
        "transition-colors " +
        (isNotRequired ? "opacity-60 bg-neutral-50/60" : "hover:bg-neutral-50")
      }
    >
      <div className="flex items-center gap-3 px-5 py-4">
        <span className="text-xs font-bold text-neutral-300 w-5 text-right flex-shrink-0 tabular-nums">
          {index}
        </span>
        <div
          className={
        ...
        <div className="flex-1 min-w-0">
          ...
          {isUploaded && rec && rec.uploadedAt ? (
            <p className="text-xs text-neutral-500 mt-0.5">
              Uploaded {formatTimestamp(rec.uploadedAt)}
              {rec.file ? " · " + formatFileSize(rec.file.size) : ""} · by{" "}
              {rec.uploadedByName ||
                (rec.uploadedBy === "coordinator" ? "Coordinator" : "You")}
            </p>
          ) : isNotRequired ? (
            <p className="text-xs text-neutral-400 mt-0.5">
              Deactivated by coordinator — not counted toward completion
            </p>
          ) : (
            <p className="text-xs text-neutral-400 mt-0.5">
          ...
          {!isNotRequired && (
            <React.Fragment>
              ...
              <button
                ...
                className={isUploaded ? btnSmSecondary : btnSmDefault}
              >
                {isUploading ? (
      ...
      {expanded && isUploaded && rec && (
        <div className="mx-5 mb-4 p-4 bg-neutral-50 border border-neutral-200 rounded-xl">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div>
              <p className="text-neutral-400 mb-0.5">File name</p>
              <p className="font-semibold text-neutral-900 truncate">
                {rec.file ? rec.file.name : "—"}
              </p>
            </div>
            <div>
              <p className="text-neutral-400 mb-0.5">File size</p>
              <p className="font-semibold text-neutral-900">
                {rec.file ? formatFileSize(rec.file.size) : "—"}
              </p>
            </div>
            <div>
              <p className="text-neutral-400 mb-0.5">Uploaded at</p>
              <p className="font-semibold text-neutral-900">
                {formatTimestamp(rec.uploadedAt)}
              </p>
            </div>
            <div>
              <p className="text-neutral-400 mb-0.5">Uploaded by</p>
              <p className="font-semibold text-neutral-900">
                {rec.uploadedByName ||
    ...
    <div className="w-full space-y-5">
      <div className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-neutral-500 mt-1">
            Host Training Establishment · {student.year} {student.semester}
          </p>
        </div>
        <div className="text-right flex-shrink-0">
          <p
      ...
      <div className="bg-white border border-neutral-200 rounded-xl px-5 py-4 shadow-sm">
        <div className="flex items-center justify-between mb-2">
          ...
          <span
            className={
              "text-xs font-bold " +
              (isComplete ? "text-success" : "text-neutral-500")
            }
          >
            {isComplete ? "Complete ✓" : "In Progress"}
          </span>
        ...
        <div className="w-full h-2.5 bg-neutral-100 rounded-full overflow-hidden">
          <div
            className={
              "h-full rounded-full transition-all duration-700 " +
              (isComplete ? "bg-success" : "bg-neutral-400")
            }
            style={{ width: pct + "%" }}
          />
        </div>
      ...
      <div className="bg-warning/8 border border-warning/30 rounded-xl p-4 flex items-start gap-3">
        <AlertTriangle className="h-4 w-4 text-warning mt-0.5 flex-shrink-0" />
        <p className="text-xs text-neutral-700 leading-relaxed">
          Upload all required HTE documents below. Accepted formats:{" "}
          <span className="font-semibold text-neutral-900">
            PDF, DOCX, JPG, PNG
          </span>{" "}
          · Max size:{" "}
  ...
  var sections = [
    {
      ...
      title: "Required OJT Documents (13 fields)",
      items: [
        "Curriculum Vitae",
        "Certificate of Registration (COR)",
        "OJT Seminar Certificate",
        "Received Copy of the OJT Recommendation Letter",
        "OJT Waiver",
        "Training Agreement",
        "Job Description",
        "Attendance Record",
        "OJT Progress Report",
        "Job Proficiency Rating / Evaluation Sheet",
        "OJT Certificate",
        "Narrative Report",
    ...
    {
      accent: "bg-warning",
      icon: <Upload className="h-4 w-4 text-warning" />,
      title: "Upload Rules",
      items: [
        "Accepted formats: PDF, DOCX, JPG, JPEG, PNG only",
        "Maximum file size: 10 MB per document",
        "You may replace a previously uploaded document at any time",
        "Upload timestamp and your identity are recorded automatically",
        "Unsupported formats or oversized files will be rejected with an error",
      ],
    },
    {
      ...
      title: "HTE Documents",
      items: [
        "Company Profile and Company Evaluation are required HTE documents",
        "Upload them just like OJT documents — PDF, DOCX, JPG, PNG up to 10 MB",
        "Both students and coordinators may upload to HTE slots",
      ],
    },
    {
      accent: "bg-neutral-300",
      icon: <MinusCircle className="h-4 w-4 text-neutral-400" />,
  ...
  return (
    <div className="w-full space-y-5">
      <div className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm flex items-center gap-3">
        <div className="h-10 w-10 rounded-xl bg-primary-500/10 border border-primary-500/20 flex items-center justify-center flex-shrink-0">
        ...
        <div>
          ...
          <p className="text-xs text-neutral-500 mt-0.5">
            Read carefully before uploading your documents
          </p>
        </div>
      </div>
              ...
              <ul className="space-y-2">
                {sec.items.map(function (item, j) {
                  return (
                    ...
                    >
                      <span className="h-1.5 w-1.5 rounded-full bg-neutral-300 flex-shrink-0 mt-[7px]" />
                      {item}
                    </li>
                  );
                })}
...
function ChangePasswordModal(props) {
  ...
  var stepState = React.useState("otp");
  ...
  var otpSentState = React.useState(false);
  var otpSent = otpSentState[0];
  var setOtpSent = otpSentState[1];
  var showPwState = React.useState(false);
  var showPw = showPwState[0];
  var setShowPw = showPwState[1];
  var showCpwState = React.useState(false);
  var showCpw = showCpwState[0];
  var setShowCpw = showCpwState[1];
  var cooldownState = React.useState(0);
    ...
    >
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl border border-neutral-200 w-full max-w-md mx-4 overflow-hidden">
        ...
        <div className="bg-gradient-primary px-6 py-5 flex items-center gap-3">
          ...
          <button
            onClick={onClose}
            className="h-8 w-8 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center transition-all cursor-pointer"
          >
        ...
        <div className="px-6 pt-4 pb-2 flex items-center gap-2">
          <div
            className={
              ...
              (step === "otp" ? "text-primary-600" : "text-success")
            }
          >
            <div
              className={
          ...
          <div
            className={
              "flex-1 h-px " +
              (step !== "otp" ? "bg-success" : "bg-neutral-200")
            }
          />
          <div
            className={
        ...
        <div className="px-6 py-4 space-y-4">
            ...
            <React.Fragment>
              <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-3">
                <div className="flex items-center gap-2 text-xs text-neutral-600">
                  ...
                  <span>
                    OTP will be sent to:{" "}
                    <strong className="text-neutral-900">{studentEmail}</strong>
                  </span>
                </div>
              </div>
              <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className={
                    btnSmDefault +
                    " flex-1 h-10 !text-sm !bg-neutral-800 !border-neutral-900 hover:!bg-neutral-700"
                  }
                >
                  {loading && otpSent ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
          ...
          {step === "done" && (
            <div className="text-center py-4">
              ...
              <button
                onClick={onClose}
                className={btnSmDefault + " h-10 !text-sm px-8"}
              >
                <span>Done</span>
              </button>
            </div>
...
function Toast(props) {
  ...
  return (
    <div
      className={
        "pointer-events-auto flex items-stretch rounded-xl border shadow-lg overflow-hidden bg-white " +
        (isError ? "border-destructive-semantic/20" : "border-success/20")
      }
    >
      <div
        className={
          "w-1 flex-shrink-0 " +
          (isError ? "bg-destructive-semantic" : "bg-success")
        }
      />
      <div className="flex items-start gap-3 px-4 py-3.5 flex-1 min-w-0">
        <div
          className={
            ...
            (isError ? "text-destructive-semantic" : "text-success")
          }
        >
          {isError ? (
