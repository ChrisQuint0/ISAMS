...
import { Monitor, Laptop, AlertTriangle } from "lucide-react";

export default function PCGridLayout({ stations, selectedPC, onSelectPC }) {
    return (
        <div className="bg-[#0f172a] border border-[#1e293b] rounded-2xl p-8 shadow-2xl relative overflow-hidden">
                ...
                {stations.map((station) => (
                    <button
                        ...
                        className={`aspect-square rounded-xl border flex flex-col items-center justify-center p-2 transition-all duration-300 relative group
                            ...
                            ${selectedPC?.id === station.id ? 'ring-2 ring-white scale-105' : 'hover:scale-105'}
                        `}
                    >
                        {/* Station Icon */}
