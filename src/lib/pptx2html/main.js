...
const renderPptx = (pptx, resultElement, thumbElement) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        // We use the shim worker for direct execution (no separate worker file needed)
        ...
        const resize = () => {
            const slides = $wrapper.children('section');
            if (slides.length === 0) return;

            const slidesWidth = Math.max(...Array.from(slides).map(s => s.offsetWidth))
            const wrapperWidth = $wrapper[0].offsetWidth

            if (slidesWidth > 0) {
                const scale = wrapperWidth / slidesWidth;
                $wrapper.css({
                    'transform': `scale(${scale})`,
                    'transform-origin': 'top left'
                });

                // Fix: Adjust the height of the result container to match the scaled height
                // This removes the "big chunk of white space" at the bottom
                const totalHeight = $wrapper[0].scrollHeight;
                $result.css('height', (totalHeight * scale) + 'px');
            }
        }
        resize()
...
function alphaNumeric(num, upperLower) {
    ...
    if (upperLower === 'upperCase') {
        aNum = (((num / 26 >= 1) ? String.fromCharCode(Math.floor(num / 26) + 64) : '') + String.fromCharCode(num % 26 + 65)).toUpperCase()
    } else if (upperLower === 'lowerCase') {
        aNum = (((num / 26 >= 1) ? String.fromCharCode(Math.floor(num / 26) + 64) : '') + String.fromCharCode(num % 26 + 65)).toLowerCase()
    }
    return aNum
}
