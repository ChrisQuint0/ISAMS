  ...
  async function genShape(node, slideLayoutSpNode, slideMasterSpNode, id, name, idx, type, order, warpObj) {
    ...
    if (shapType !== undefined || custShapType !== undefined) {
      ...
      if (clrFillType === 'GRADIENT_FILL') {
        ...
        imgFillFlg = true
        const svgBgImg = await getSvgImagePattern(fillColor, shpId, warpObj)
        // fill="url(#imgPtrn)"
...
*/
  ...
  async function genTextBody(textBodyNode, slideLayoutSpNode, slideMasterSpNode, type, warpObj) {
    ...
    if (textBodyNode['a:p'].constructor === Array) {
      ...
      for (let i = 0; i < textBodyNode['a:p'].length; i++) {
        ...
        if (rNode === undefined) {
          // without r
          text += await genSpanElement(pNode, slideLayoutSpNode, slideMasterSpNode, type, warpObj)
        } else if (rNode.constructor === Array) {
          // with multi r
          for (let j = 0; j < rNode.length; j++) {
            text += await genSpanElement(rNode[j], slideLayoutSpNode, slideMasterSpNode, type, warpObj)
            // ////////////////Amir////////////
          ...
          // with one r
          text += await genSpanElement(rNode, slideLayoutSpNode, slideMasterSpNode, type, warpObj)
        }
        text += '</div>'
      }
    } else {
  ...
  async function genSpanElement(node, slideLayoutSpNode, slideMasterSpNode, type, warpObj) {
    const slideMasterTextStyles = warpObj['slideMasterTextStyles']
    ...
    if (trNodes.constructor === Array) {
      for (let i = 0; i < trNodes.length; i++) {
        ...
        if (tcNodes.constructor === Array) {
          ...
          const text = await genTextBody(tcNodes['a:txBody'], undefined, undefined, undefined, warpObj)
          // Cells Style : TODO /////////////Amir
...
*/
  ...
  async function getShapeFill(node, isSvgMode, warpObj) {
    ...
    if (fillType === 'NO_FILL') {
      ...
      const shpFill3 = node['p:spPr']['a:blipFill']
      fillColor = await getPicFill('slideBg', shpFill3, warpObj)
    }
  ...
  function getSchemeColorFromTheme(schemeClr, sldMasterNode) {
    ...
    if (slideLayoutClrOvride) {
      switch (schmClrName) {
        ...
        case 'bg1':
        case 'bg2': {
          if (slideLayoutClrOvride[schmClrName]) {
