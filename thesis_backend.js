...
// Health check
app.get("/", (req, res) =>
  res.json({ status: "Thesis Backend Running", port: 3001 }),
);
...
async function loadToken() {
  ...
  if (oauth2Client.isTokenExpiring()) {
    try {
      console.log("[Auth] Refreshing expired Google Drive token");
      ...
      console.error("[Auth] Failed to refresh Google Drive token:", err);
      return null;
    }
  }
...
app.post("/api/thesis/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    const folderId =
      (await getThesisSetting("thesis_root_folder_id")) ||
      DEFAULT_THESIS_ROOT_FOLDER_ID;
    ...
    if (auth && thesis.files) {
      ...
      for (const file of thesis.files) {
        if (file.storage_path) {
          try {
            console.log(
              `[Delete] Removing file from GDrive: ${file.storage_path}`,
            );
            await drive.files.delete({ fileId: file.storage_path });
          } catch (driveError) {
            console.error(
              `[Delete] Error deleting file ${file.storage_path}:`,
              driveError.message,
            );
